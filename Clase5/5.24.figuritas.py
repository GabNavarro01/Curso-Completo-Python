...
def albumes_incompleto(A, cantidad_albumes):
    return min(A) != cantidad_albumes

def cuantos_paquetes_cooperar(figus_total, figus_paquete,cantidad_albumes):
    album = crear_album(figus_total)

    while albumes_incompleto(album,cantidad_albumes):
        paquete = comprar_paquete(figus_total, figus_paquete)
        for figu in paquete:
            album[figu] +=1
...
def cooperar(n_repeticiones,figus_total,figus_paquete, n_personas):
    a = cuantos_paquetes_cooperar(figus_total, figus_paquete, n_personas)
    return a
...
# cooperacion = cooperar(n_repeticiones,figus_total,figus_paquete,5)
