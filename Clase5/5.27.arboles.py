...
colores = {
    "Eucalipto": "purple",
    "Palo borracho rosado": "green",
    "Jacarandá": "orange"
}

for esp in especies_arboles:
    arr = np.array(medidas[esp])
    x = arr[:,0]   # diametro
    y = arr[:,1]   # altura
    plt.scatter(x, y, color=colores[esp], alpha=0.1, label=esp)

plt.xlabel("diametro (cm)")
plt.ylabel("alto (m)")
plt.title("Diametro vs Altura de distintas especies")
plt.xlim(0,50) 
plt.ylim(0,200) 
plt.legend()
plt.show()
